...
module.exports = function({ minified, es6, coverage, tests, server }) {
  return {
    input: server ? 'demo/index.ts' : 'src/bitbucket.ts',
    external: tests || server ? [] : ['@salte-auth/salte-auth'],
    output: {
      file: `dist/bitbucket${minified ? '.min' : ''}.${es6 ? 'mjs' : 'js'}`,
      format: es6 ? 'es' : 'umd',
      ...
      banner: deindent`
         ...
         *
         * @license MIT (https://github.com/salte-auth/bitbucket/blob/master/LICENSE)
         *
